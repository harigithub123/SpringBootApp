...
import com.billing.dao.RoleRepository;
...
import com.billing.request.SignupRequest;
import com.billing.response.JwtResponse;
import com.billing.response.MessageResponse;
import com.billing.security.JwtUtils;
import com.billing.service.impl.UserDetailsImpl;
		...
		if (strRoles == null) {
			...
			strRoles.forEach(role -> {
				...
				case "admin":
					...
					break;
				case "mod":
					Role modRole = roleRepository.findByName(ERole.ROLE_ACCOUNT)
							.orElseThrow(() -> new RuntimeException("Error: Role is not found."));
					roles.add(modRole);
